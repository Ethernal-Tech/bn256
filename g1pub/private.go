...
// PublicKey returns the public key from the PrivateKey (on G1)
func (p *PrivateKey) PublicKey() *PublicKey {
	g1 := new(bn256.G1).ScalarMult(g1Point, p.s)

	return &PublicKey{g1: g1}
}
...
// Sign generates a simple signature of the given message (on G2)
func (p *PrivateKey) Sign(message []byte) (*Signature, error) {
	if len(message) != 32 {
		return nil, ErrInvalidMessageLength
	}

	point := hashToG2Point([32]byte(message))
	g2 := new(bn256.G2).ScalarMult(point, p.s)
