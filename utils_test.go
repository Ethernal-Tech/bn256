...
func Test_SingleSign(t *testing.T) {
	...
	isOk := signature.Verify(key.PublicKey(), validTestMsg, expectedDomain)
	assert.True(t, isOk)
	...
	isOk = signature.Verify(key.PublicKey(), invalidTestMsg, unexpectedDomain)
	assert.False(t, isOk)
}
...
func Test_AggregatedSign(t *testing.T) {
	...
	keys, err := GeneratePrivateKeys(participantsNumber) // create keys for validators
	require.NoError(t, err)
